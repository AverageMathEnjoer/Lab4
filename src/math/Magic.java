...
public class Magic {
    ...
    public static int randomStat(int min, int max) throws InvalidConfinesException {
        if(max > min){
            return (int) ((max - min) * Math.random() + min);
        }
        else throw new InvalidConfinesException("Вы максимум и минимум спутали", min, max);
    }
